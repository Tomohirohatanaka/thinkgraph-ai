...
JSON形式のみ返答:
{
  ...
  "core_text": "学習の核心内容・重要ポイントを詳しく（800〜1500文字。事実・定義・因果関係・手順など教材の中核を正確に抜き出すこと）",
...
## modeの選択基準（コンテンツの主な学習目的で判断）
- whynot: 「なぜそうなるか」を理解することが目的。因果関係・メカニズム・原理の解明が中心のコンテンツ
  例: 「なぜ地球は温暖化するのか」「光合成が必要な理由」「経済危機の原因」
- vocabulary: 専門用語・定義・概念名の正確な理解が目的。用語集・定義集・略語解説が中心
  例: 「DNA/RNA/タンパク質の定義」「法律用語の解説」「プログラミング用語集」
- concept: 概念・理論の全体像と関係性を理解することが目的。体系的な知識構造が中心
  例: 「民主主義とは」「進化論の仕組み」「機械学習の基礎理論」
- procedure: 手順・プロセス・やり方を理解することが目的。順序が重要なコンテンツ
  例: 「実験の手順」「料理のレシピ」「アルゴリズムの実装手順」「申請の流れ」

判断に迷う場合は、コンテンツの中で最も多くの分量を占める要素で判定する。`;
}
